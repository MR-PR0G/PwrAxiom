...
static unsigned long long prev_energy_uj = 0;
static gint64 prev_time_ms = 0;
...
static void read_system_stats() {
    ...
    if (!rapl) rapl = fopen("/sys/class/hwmon/hwmon0/energy1_input", "r");
    
    if (rapl) {
        unsigned long long energy;
        if (fscanf(rapl, "%llu", &energy) == 1) {
            gint64 now = g_get_monotonic_time() / 1000;
            if (prev_time_ms > 0 && energy > prev_energy_uj) {
                double seconds = (now - prev_time_ms) / 1000.0;
                cpu_watts = (energy - prev_energy_uj) / 1000000.0 / seconds;
            }
    ...
    if (bat_f && cap_f) {
        ...
        char bat_text[256];
        if (cpu_watts > 0.0) {
            snprintf(bat_text, sizeof(bat_text), "<span size='large' weight='bold'>AC</span>\n<span size='x-small'>Package: %.1fW</span>", cpu_watts);
        } else {
