...
void state_unref(ExecutionState *state) {
    if (g_atomic_int_dec_and_test(&state->ref_count)) {
        ...
        if (state->status_file_path) g_free(state->status_file_path);
        g_free(state);
    }
}
...
void detect_system() {
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            if (strncmp(line, "ID=", 3) == 0) {
                profile.distro = g_strdup(line + 3);
    ...
    if (!profile.distro) profile.distro = g_strdup("Linux");
    ...
    if (gpu_f) {
        ...
        while (fgets(line, sizeof(line), gpu_f) && count < 2) {
            ...
            if (strlen(temp_gpu) > 0) strcat(temp_gpu, "\n");
            strcat(temp_gpu, line);
            count++;
        }
        pclose(gpu_f);
    }
    if (strlen(temp_gpu) == 0) strcpy(temp_gpu, "Unknown GPU");
    profile.gpu_info = g_strdup(temp_gpu);
}
...
void apply_theme(int index) {
    ...
    gchar *css = g_strdup_printf(
        ...
        "button.planet { border-radius: 10px; min-width: 115px; min-height: 75px; background: #151515; border: 1px solid %s; box-shadow: 0 0 15px %s; color: white; font-weight: bold; font-size: 11px; transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); }"
        ...
        ".about-text { color: #888; font-size: 12px; margin-top: 15px; margin-bottom: 10px; }"
        "button.dev-link { color: #ffffff; background: #222222; border-radius: 8px; font-size: 11px; font-weight: bold; padding: 8px; border: 1px solid #333; transition: all 0.2s; }"
        "button.dev-link:hover { background: %s; border-color: %s; box-shadow: 0 0 10px %s; }"
        "button.dev-link label { text-decoration: none; }"
        ...
        t_title[index], t_act_b[index], t_act_b[index], t_shadow[index]
    );
...
void close_sidebar_on_click(GtkGestureClick *gesture, int n_press, double x, double y, gpointer data) {
    GtkRevealer *rev = GTK_REVEALER(data);
    GtkWidget *window = gtk_widget_get_ancestor(GTK_WIDGET(rev), GTK_TYPE_WINDOW);
    if (!window) return;
...
static gboolean update_status_from_log(gpointer user_data) {
    ...
    if (!state->log_file && state->status_file_path) {
        state->log_file = fopen(state->status_file_path, "r");
        if (!state->log_file) return G_SOURCE_CONTINUE;
    }

    char line[256];
    while (state->log_file && fgets(line, sizeof(line), state->log_file)) {
        if (g_str_has_prefix(line, "STATUS:")) {
            if (!state->has_started) {
                gtk_widget_set_visible(state->btn_box, FALSE);
                gtk_widget_set_visible(state->progress_box, TRUE);
...
void run_power_command_async(ExecutionState *state) {
    const char *config_home = g_get_user_config_dir();
    state->status_file_path = g_strdup_printf("%s/pwraxiom/status.log", config_home);
    FILE *temp_f = fopen(state->status_file_path, "w");
    ...
    char *script_path = pwraxiom_get_script_path(state->target_mode);
    char core_arg[16];
    snprintf(core_arg, sizeof(core_arg), "%d", profile.cores);

    gchar *argv[] = {"pkexec", "/bin/bash", script_path, state->status_file_path, core_arg, install_cmd, NULL};
    GError *error = NULL;
    ...
    if (proc) {
        state->timer_id = g_timeout_add(100, update_status_from_log, state);
...
void confirm_execution(GtkWidget *widget, gpointer data) {
    ...
    gtk_button_set_label(GTK_BUTTON(widget), "Authenticating...");
...
void show_mode_popup(GtkWindow *parent, const char *name, const char *desc) {
    ...
    GtkWidget *status_label = gtk_label_new("Applying settings...");
    ...
    GtkWidget *btn_close_err = gtk_button_new_with_label("OK");
    ...
    state->popup = popup;
    state->btn_box = btn_box;
    state->progress_box = progress_box;
    state->progress = 0.0;
    state->has_started = FALSE;
    state->is_finished = FALSE;
    state->log_file = NULL;
    state->status_file_path = NULL;
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    GtkWidget *link_btn = gtk_link_button_new_with_label("https://github.com/MR-PR0G", "Project Page");
    gtk_widget_add_css_class(link_btn, "dev-link");
    gtk_box_append(GTK_BOX(sidebar_box), link_btn);
    ...
    const char *labels[] = {"Ultra\nPerformance", "Performance", "Balanced", "Save", "Ultra\nSave"};
    ...
    GtkWidget *bottom_lbl = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(bottom_lbl), "<span size='small' color='#555555'>Developed by Mr Prog</span>");
    gtk_widget_set_halign(bottom_lbl, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(bottom_lbl, GTK_ALIGN_END);
    gtk_widget_set_margin_bottom(bottom_lbl, 15);
    gtk_overlay_add_overlay(GTK_OVERLAY(overlay), bottom_lbl);
...
void free_profile_strings() {
    if (profile.distro) g_free(profile.distro);
    if (profile.gpu_info) g_free(profile.gpu_info);
}
